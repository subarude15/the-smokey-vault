...
- **PR168** added a keeper-only brew-sheet data layer (`brew_recipes`, `brew_sessions`, `BrewRecipeDocument`) separate from the homebrew `brews` log. **PR169** parses pasted brewing instructions into that document through the existing AI failover path and does not save. **PR170** adds the keeper-only Brew Sheet Builder (paste, analyze, review, explicit save). HTML preview and PDF export remain later slices.
