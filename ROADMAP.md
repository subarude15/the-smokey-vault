...
- **PR168** added a keeper-only brew-sheet data layer (`brew_recipes`, `brew_sessions`, `BrewRecipeDocument`) separate from the homebrew `brews` log. **PR169** parses pasted brewing instructions into that document through the existing AI failover path and does not save. **PR170** adds the keeper-only Brew Sheet Builder (paste, analyze, review, explicit save). **PR171** adds the saved-recipe library, open/edit, and Brew Again (a numbered `brew_sessions` row, not a recipe copy). **PR172** adds the print-style brew sheet preview through one canonical document renderer. **PR173** downloads a keeper-only US Letter PDF of a saved recipe by printing that same document with system Chromium. The PDF is not stored.
