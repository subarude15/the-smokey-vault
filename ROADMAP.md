...
12. **PR147 — Visual system / Smokey Barrel branding polish** (done)
   ...
   - Typography pairing via tokens (`--font-display`/`--font-serif`/`--font-sans`): a blackletter (`Manufacturing Consent`, pinned to its single 400 weight, `font-synthesis:none`) is used only for brand moments and major headers (hero wordmark, page titles, section/feature/empty/modal headings); Playfair Display stays for content titles (product/cocktail names); and `JetBrains Mono` is used for all body/controls/metadata for a controlled industrial-punk edge.
