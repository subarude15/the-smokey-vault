...
11. **PR146 — Gallery comments + up/down voting** (done)
   ...
   - New normalized tables `gallery_comments` and `gallery_votes` (media-scoped indexes; `UNIQUE(media_id, voter_key)` enforces one active vote per voter). Comments/votes are deleted transactionally inside `deleteGalleryMedia`; album operations never touch them.
   - Duplicate-vote safeguard: the anonymous voter key is derived server-side (`deriveGalleryVoterKey` = HMAC of the client's existing per-device `smokey-voter` token plus IP/User-Agent, keyed with the session secret). The raw key is never returned; same-direction re-vote toggles off, up↔down reuses the single row.
