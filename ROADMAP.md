...
20. **PR159 — Homepage Brand Hero & Filigree Refinement** (done)
   ...
   - Added a restrained decorative hop-vine filigree (`client/src/HopFiligree.tsx`, also traced vector) flowing down the upper-right of the hero and fading before the CTAs. It is `aria-hidden`, non-interactive (`pointer-events:none`), token-tinted via `--line` (charcoal/gunmetal on Dark, muted neutral on Light) with a copper touch, and clipped by the hero so it never causes horizontal overflow. Verified at 320/360/390/430px.
