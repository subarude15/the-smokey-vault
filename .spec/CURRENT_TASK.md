...
- Tokenized typography: `--font-display` (`Grenze Gotisch`, readable blackletter) for brand moments + major headers only (hero wordmark, page titles, section/feature/empty/modal headings); `--font-serif` (Playfair) for content titles; `--font-sans` (Outfit) for body/controls/metadata.
