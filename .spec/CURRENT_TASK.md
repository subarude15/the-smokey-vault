...
- Tokenized typography: `--font-display` (`Manufacturing Consent` blackletter, weight 400, `font-synthesis:none`) for brand moments + major headers only (hero wordmark, page titles, section/feature/empty/modal headings); `--font-serif` (Playfair) for content titles; `--font-sans` (`JetBrains Mono`) for all body/controls/metadata.
