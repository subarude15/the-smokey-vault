...
- **PR151** — Bottle Library media and loading-state polish (branch `cursor/pr151-bottle-library-media-loading-95d9`). Spirit cards use one stable media frame (`SpiritCardMedia`) with a branded SB-mark placeholder for missing/broken `image_url`s (no Lucide bottle icon, no layout jump, no Keeper media mutation on `onError`). Inventory loading / loaded-empty / loaded-populated / error are distinct: a lightweight `SpiritShelfSkeleton` shows while the initial Spirits request is pending so Guests never see “Nothing on the shelf” mid-load; empty copy appears only after a successful zero-row response; errors keep Retry and never render as an empty shelf. Pure `inventoryViewState` helper + focused media/state tests. No flavor facets (PR152), shell chrome (PR153), upload/enrichment, or backend/schema changes.
