...
test("UI source: Keeper Find photo only when image missing; cards render image_url", () => {
  ...
  const photoSrc = readFileSync(join(root, "client/src/CocktailPhoto.tsx"), "utf8");
  assert.match(appSrc, /<CocktailPhoto/);
  assert.match(photoSrc, /recipe-hero/);
  assert.match(photoSrc, /alt=\{name\}/);
  ...
  assert.match(cssSrc, /\.recipe-modal \.recipe-hero\{[^}]*object-fit:contain/);
