 ...
 *
 * Privacy: the anonymous voter key is derived server-side (HMAC of limited
 * device/request context with the session secret) and is never returned to the
 * client. Guest responses expose only what the feature needs — comment author,
 * text, timestamp, and aggregate vote counts plus the caller's own vote.
 */

export const MAX_GALLERY_COMMENT = 500;
export const MAX_GALLERY_COMMENT_AUTHOR = 40;
/** Cap on stored voter-key length (a hex HMAC is 64 chars). */
const MAX_VOTER_KEY = 128;
...
/** Guest-safe comment shape. `voter_key` is stored but never serialized out. */
...
/**
 * Derive an opaque, per-device anonymous voter key from limited request/device
 * context, keyed with the session secret. A private household bar shares one LAN
 * IP, so a per-device token (the client's existing `smokey-voter` localStorage
 * id) is the primary signal; IP + User-Agent add modest entropy. The result is
 * an HMAC digest — the raw context is never stored and the key is never returned
 * to the client, so duplicate-vote protection data stays entirely server-side.
 */
export function deriveGalleryVoterKey(
  ...
  const ip = (parts.ip ?? "").trim();
  const userAgent = (parts.userAgent ?? "").trim().slice(0, 400);
...
export function addGalleryComment(
  mediaId: number,
  input: { body: string; author?: string; voterKey?: string }
): GalleryComment {
  ...
  if (!galleryMediaExists(mediaId)) throw new GallerySocialError("That item is already gone", 404);
  const voterKey = (input.voterKey ?? "").slice(0, MAX_VOTER_KEY);
  const result = db.prepare(
    "INSERT INTO gallery_comments(media_id, author, body, voter_key) VALUES(?, ?, ?, ?)"
  ).run(mediaId, author, text, voterKey);
  return db.prepare(
