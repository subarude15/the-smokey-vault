...
function identityContext(candidate: BottleCandidate) {
  return {
...
${JSON.stringify(identityContext(request.candidate), null, 2)}

Image URL (for reference): ${request.imageUrl}

Answer ONLY with JSON booleans:
- correct_product: true if the image shows this same product/brand bottle
...
export async function verifyProductImage(request: ImageVerifyRequest): Promise<VisionVerification | null> {
  ...
  return parseVisionVerification(data.message?.content ?? null);
}
