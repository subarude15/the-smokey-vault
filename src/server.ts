...
import { callLlm, AiRequestError, resolveAiConfig } from "./ai_client.js";
import { type AiProviderConfig, buildAiFailoverChain } from "./ai_providers.js";
...
app.post<{ Body: { upc?: string; hit?: BottleSearchHit } }>("/api/beer/remember", {
  ...
  const canonicalUpc = canonicalGtin(upc);
  if (!canonicalUpc) return reply.code(400).send({ error: "Valid GTIN required" });
  await rememberBeerFromHit(canonicalUpc, hit);
  return { ok: true };
});
